...
- **Infrastructure-as-code:** An Azure Bicep template to deploy the solution and its required components using either `azd` or one-click deploy.
...
| Example  | Description | Pipeline summary |
| ---------|---------|---------|
|**Form Field Extraction with Confidence Scores & Bboxes**<br>[Code](/function_app/bp_form_extraction_with_confidence.py)| Extracts key information from a PDF form, and returns field-level and overall confidence scores and whether human review is required. | - PyMuPDF (PDF -> Image)<br>- Document Intelligence (PDF -> text)<br>- GPT-4o (text + image input)<br>- Post-processing: confidence scores and bounding boxes<br>- Returned structured JSON |
|**Summarize Text**<br>[Code](/function_app/bp_summarize_text.py)| Summarizes text input into a desired style and number of output sentences. | - GPT-4o (text input + style/length instructions)<br>- Return raw text |
|**City Names Extraction (Doc Intelligence)**<br>[Code](/function_app/bp_doc_intel_extract_city_names.py)| Uses GPT-4o to extract all city names from a given PDF/image + text extracted by Document Intelligence. | - Document Intelligence (Image/PDF -> text)<br>- GPT-4o (text + image input)<br>- Return JSON array of city names |
|**City Names Extraction (PyMuPDF)**<br>[Code](/function_app/bp_pymupdf_extract_city_names.py)| Uses GPT-4o to extract all city names from a given PDF/image + text extracted by PyMuPDF. | - PyMuPDF (Image/PDF -> text)<br>- GPT-4o (text + image input)<br>- Return JSON array of city names |
|**Doc Intelligence Only**<br>[Code](/function_app/bp_doc_intel_only.py)| A simple pipeline that calls Document Intelligence and returns the full response and raw text content. | - Document Intelligence (Image/PDF -> text)<br>- Return Structured JSON object |
...
[![Deploy to Azure](https://aka.ms/deploytoazurebutton)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2FAzure27%2Fmultimodal-ai-llm-processing-accelerator%2Fmain%2Finfra%2Fmain.json)

`Deploy to Azure` offers a one click deployment without cloning the code. If you are cloning and modifying the repo, it is best to deploy using `azd` using the instructions below.

### Option 2: Deploying to Azure with `azd`
